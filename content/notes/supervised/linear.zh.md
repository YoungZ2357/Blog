...
| ![](/Blog/images/lnr1.png) | ![](/Blog/images/lnr2.png) |
| ------------- | ------------- |
...
$$
\frac{\partial}{\partial\theta_1}\sum_{i=1}^{n}(y_i-\theta_0-\theta_1x_i-\theta_2x_i^2)^2 = 2\sum_{i=1}^n (y_i - \theta_0 - \theta_1 x_i - \theta_2 x_i^2)(-x_i)
$$

$$
\frac{\partial}{\partial\theta_0}\sum_{i=1}^{n}(y_i-\theta_0-\theta_1x_i-\theta_2x_i^2)^2 = 2\sum_{i=1}^n (y_i - \theta_0 - \theta_1 x_i - \theta_2 x_i^2)(-1)
$$
...
似然函数的意义是：在给定观测数据 $ y $ 的情况下，模型参数 $ \pmb\theta $ 出现的可能。
...
> 3.26 补：在非大数据场景下，闭式解的代价通常是可以接受的，若$n=10000$，输入输出维度为3000，有5000参数，在 i9-k11900k上求取闭式解，大概会达到秒级别的理论运算速度。梯度下降大多数用于复杂网络结构和无闭式解优化目标（如逻辑回归）
